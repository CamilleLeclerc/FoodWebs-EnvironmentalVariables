...
get_relative_contribs <- function(gam_model) {
  ...
  gam_model = gam(formula(gam_model), data = model_data, family = gam_model$family, select = FALSE, method = "REML")
  ...
  devs <- map_dbl(terms$term.labels, function(term) {
    ...
    new_model <- gam(formula = new_formula, sp = smooth_par, data = model_data, family = gam_model$family, select = FALSE, method = "REML")
  ...
  null_model = gam(formula = as.formula(null_formula), data = model_data, family = gam_model$family, select = FALSE, method = "REML")
